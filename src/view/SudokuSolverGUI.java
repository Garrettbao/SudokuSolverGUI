...
import controller.SolverWorker;
...
public class SudokuSolverGUI extends JFrame {
    ...
    private void solveFromArrayInput() {
        ...
        try {
            char[][] boardArray = Parser.parseBoard(input);
            SudokuBoard board = new SudokuBoard(boardArray);
            if (!board.isInitialBoardValid()) {
            ...
            displayInitialBoard(board.getBoard());

            // Switch to "Solved Sudoku" tab
            tabbedPane.setSelectedIndex(2);

            // Start solving in background
            SolverWorker worker = new SolverWorker(board, sleepTime, this, stopButton);
            currentSolverWorker = worker;
            worker.execute();
    ...
    private void solveFromGridInput() {
        try {
            char[][] boardArray = getBoardFromGridInput();
            SudokuBoard board = new SudokuBoard(boardArray);
            if (!board.isInitialBoardValid()) {
                JOptionPane.showMessageDialog(this,
                        "The initial Sudoku board is invalid. Please correct the duplicates and try again.",
     ...
     */
    public void updateSolvedCell(int row, int col, char value, boolean isPlaced) {
    ...
    /**
     * Sets the current SolverWorker instance.
     *
     * @param worker The SolverWorker instance.
     */
    public void setCurrentSolverWorker(SolverWorker worker) {
        this.currentSolverWorker = worker;
    }

    /**
     * Gets the current SolverWorker instance.
     *
     * @return The current SolverWorker instance.
     */
    public SolverWorker getCurrentSolverWorker() {
        return currentSolverWorker;
    }
}
